...
def on_class_change(sem, i):
    ...
    if st.session_state.sync_toggle:
        if sem == "S1":
            # Push S1 change to S2
            st.session_state[f"S2c{i}_val"] = new_val
        elif sem == "S2":
            # If S2 changes and doesn't match S1 anymore, KILL SYNC
...
if st.session_state.step == 1:
    ...
    if sync_ui != st.session_state.sync_toggle:
        ...
        if sync_ui: # Force immediate sync when turned on
    ...
    def grade_row(sem, i, start_c):
        ...
        curr = st.session_state.get(f"{sem}c{i}_val", "")
        with c_sel:
    ...
    with col_r:
        ...
        b3, b4 = st.columns(2)
        # Allow adding/removing S2 manually ONLY if sync is off
    ...
    if st.button("📊 Calculate Final GPA", use_container_width=True):
        st.metric("Your GPA", f"{calculate_gpa(s1_data, s2_data)}")
        st.balloons()
