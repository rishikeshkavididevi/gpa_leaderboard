...
def calculate_gpa(s1_data, s2_data):
    ...
    for row in (s1_data + s2_data):
        ...
        # Leander ISD weighted logic (approximate 4.0 scale + weights)
...
def on_class_change(sem, i):
    widget_key = f"{sem}c{i}_widget"
    new_val = st.session_state[widget_key]
    st.session_state[f"{sem}c{i}_val"] = new_val # Backing store
    
    if st.session_state.sync_toggle:
        if sem == "S1":
            # If S1 changes, force S2 to match
            st.session_state[f"S2c{i}_val"] = new_val
        elif sem == "S2":
            # AUTO-KILL: If S2 manually changes and doesn't match S1, turn off sync
...
if st.session_state.step == 1:
    ...
    if sync_ui != st.session_state.sync_toggle:
        ...
        if sync_ui: # Turned ON: copy S1 values to S2
    ...
    with col_r:
        ...
        # S2 Add/Remove buttons work only if sync is off
