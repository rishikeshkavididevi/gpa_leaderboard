...
# 2. BRANDING (Defined as a clean string)
hide_st_style = """
            <style>
...
if st.session_state.step == 1:
        ...
        for j, cyc in enumerate(cycles):
            with cols[j+1]:
                if cyc > CURRENT_CYCLE:
                    ...
                    val = st.text_input(f"C{cyc}", value="", key=f"{sem}g{cyc}_{i}", disabled=is_na)
                    grades.append("N/A" if is_na else val)
        return cls, grades
    ...
    if st.button("📊 Calculate Final Breakdown", use_container_width=True):
        ...
        if all_res:
            ...
            st.metric("Projected GPA", f"{df['GPA'].mean():.4f}")
            st.table(df)
