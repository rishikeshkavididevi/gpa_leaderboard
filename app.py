...
with col1:
    ...
    for i in range(st.session_state.num_S1):
        ...
        cls = c1.selectbox(f"Class {i+1}", [""] + ALL_CLASSES, key=f"S1c{i}", on_change=save_user_data, label_visibility="collapsed")
        g1 = c2.text_input("C1", key=f"S1g{i}_1", placeholder="C1", on_change=save_user_data, label_visibility="collapsed")
        g2 = c3.text_input("C2", key=f"S1g{i}_2", placeholder="C2", on_change=save_user_data, label_visibility="collapsed")
        g3 = c4.text_input("C3", key=f"S1g{i}_3", placeholder="C3", on_change=save_user_data, label_visibility="collapsed")
...
with col2:
    ...
    for i in range(st.session_state.num_S2):
        ...
        r = st.container()
        c1, c2, c3, c4 = r.columns([3, 1, 1, 1]) # Fixed alignment here
        cls = c1.selectbox(f"S2 Class {i+1}", [""] + ALL_CLASSES, key=f"S2c{i}", on_change=save_user_data, label_visibility="collapsed")
        g4 = c2.text_input("C4", key=f"S2g{i}_4", placeholder="C4", on_change=save_user_data, label_visibility="collapsed")
        g5 = c3.text_input("C5", key=f"S2g{i}_5", placeholder="C5", on_change=save_user_data, label_visibility="collapsed")
        g6 = c4.text_input("C6", key=f"S2g{i}_6", placeholder="C6", on_change=save_user_data, label_visibility="collapsed")
...
# --- CALCULATION BUTTON ---
st.divider()
if st.button("Calculate Final Analytics", type="primary", use_container_width=True):
    gpa1 = calculate_sem_gpa(s1_data)
    gpa2 = calculate_sem_gpa(s2_data)
    
    if gpa1 > 0 and gpa2 > 0:
        final_gpa = (gpa1 + gpa2) / 2
    else:
        final_gpa = gpa1 if gpa1 > 0 else gpa2
